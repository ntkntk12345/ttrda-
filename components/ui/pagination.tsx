...
export default function Pagination({ currentPage, totalPages, baseUrl, extraParams, className }: PaginationProps) {
    ...
    const renderPageNumbers = () => {
        ...
        if (startPage > 1) {
            pages.push(
                <Link
                    key={1}
                    href={buildUrl(baseUrl, 1, extraParams)}
        ...
        for (let i = startPage; i <= endPage; i++) {
            pages.push(
                <Link
                    key={i}
                    href={buildUrl(baseUrl, i, extraParams)}
                    className={cn(
        ...
        if (endPage < totalPages) {
            ...
            pages.push(
                <Link
                    key={totalPages}
                    href={buildUrl(baseUrl, totalPages, extraParams)}
                    className="w-10 h-10 flex items-center justify-center rounded-lg border bg-white/5 border-white/10 text-gray-400 hover:bg-white/10 hover:text-white hover:border-white/20 transition-all font-medium"
                >
                    {totalPages}
                </Link>
    ...
    return (
        <div className={cn("flex items-center justify-center gap-2 mt-12 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700 select-none", className)}>
            <Link
                href={currentPage > 1 ? buildUrl(baseUrl, currentPage - 1, extraParams) : '#'}
                className={cn(
            ...
            <Link
                href={currentPage < totalPages ? buildUrl(baseUrl, currentPage + 1, extraParams) : '#'}
                className={cn(
                    "h-10 px-4 flex items-center gap-2 rounded-lg border bg-white/5 border-white/10 text-gray-400 transition-all font-medium",
